...
def plotgraph(self, ctx):
    global eje_x_label, eje_y_label, titulo_grafico, mostrar_grid, color_linea, color_puntos, tamaño_punto
    ...
    for i in range(len(puntos) - 1):
        x1, y1 = puntos[i]
        x2, y2 = puntos[i + 1]
        lineas += f'<line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" stroke="{color_linea}" stroke-width="2" />\n'

    # ========== SVG: PUNTOS ==========
    puntos_svg = "".join([f'<circle cx="{x}" cy="{y}" r="{tamaño_punto}" fill="{color_puntos}"/>\n' for x, y in puntos])
    ...
    etiqueta_eje_x = f'<text x="{width // 2}" y="{height - 5}" font-size="12" text-anchor="middle">{eje_x_label}</text>' if eje_x_label else ''
    etiqueta_eje_y = f'<g transform="translate(20,{height // 2}) rotate(-90)"><text font-size="12" text-anchor="middle">{eje_y_label}</text></g>' if eje_y_label else ''
    etiqueta_titulo = f'<text x="{width // 2}" y="20" font-size="14" text-anchor="middle" font-weight="bold">{titulo_grafico}</text>' if titulo_grafico else ''
...
'''
    ...
    # ========== GUARDAR EN CARPETA DEL TEST ==========
    ruta_script = os.path.abspath(sys.argv[1])
    carpeta_destino = os.path.dirname(ruta_script)
    nombre_base = os.path.splitext(os.path.basename(ruta_script))[0]
    ...
    print(f"✅ Gráfico generado en: {ruta_svg}")

    # Reiniciar etiquetas y grid para la próxima gráfica
    eje_x_label = ""
    eje_y_label = ""
    titulo_grafico = ""
    mostrar_grid = False
...
def set_bar_values(self, ctx):
    global mostrar_valores_barras
    valor = self.visit(ctx.expr())
    if not isinstance(valor, bool):
        raise Exception("barValues must be True or False")
    mostrar_valores_barras = valor

def plot_bar(self, ctx):
    ...
    if titulo_grafico:
        svg += f'<text x="{width // 2}" y="20" font-size="14" font-weight="bold" text-anchor="middle">{titulo_grafico}</text>\n'
    if eje_y_label:
        svg += f'<g transform="translate(20,{height // 2}) rotate(-90)"><text font-size="12" text-anchor="middle">{eje_y_label}</text></g>\n'
    ...
    carpeta_destino = os.path.dirname(ruta_script)
    nombre_base = os.path.splitext(os.path.basename(ruta_script))[0]
    ruta_svg = os.path.join(carpeta_destino, f"grafico_{nombre_base}.svg")

    with open(ruta_svg, "w") as f:
        f.write(svg)

    print(f"✅ Gráfico de barras generado en: {ruta_svg}")
...
# ========== GRÁFICO DE PASTEL ==========
leyenda_pastel = None  # global para el texto de la leyenda

def set_legend(self, ctx):
    global leyenda_pastel
    valor = self.visit(ctx.expr())
    if not isinstance(valor, str):
        raise Exception("legend must be a string")
    leyenda_pastel = valor

def plot_pie(self, ctx):
    args = ctx.expr()
    if len(args) != 2:
    ...
    for i, (label, val) in enumerate(zip(etiquetas, valores)):
        angle = val / total * 360
  ...
  <tspan x="{leyenda_x}" dy="15">Programación Populares</tspan>
    ...
    print(f"✅ Gráfico de pastel generado en: {ruta_svg}")

    leyenda_pastel = None  # reset
